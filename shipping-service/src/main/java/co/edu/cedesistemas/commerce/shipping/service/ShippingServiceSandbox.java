...
public class ShippingServiceSandbox implements IShipmentService {
    ...
    public Shipment deliver(String id) {
        Shipment dummy = createDummyShipment(UUID.randomUUID().toString(), RandomStringUtils.randomNumeric(5));
        dummy.setStatus(ShipmentStatus.DELIVERED);
        return dummy;
    }

    @Override
    public Shipment cancel(Shipment.Cancel shipment) {
        Shipment dummy = createDummyShipment(UUID.randomUUID().toString(), RandomStringUtils.randomNumeric(5));
        dummy.setStatus(ShipmentStatus.CANCELLED);
        return dummy;
    }
