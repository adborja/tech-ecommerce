...
public class StreamDemoTest {
    ...
    public void testRectangleStreams() {
        ...
        */
        ...
        List<Rectangle> result = rectangles.stream().filter(r -> r.area()<=150)
                .filter(x -> x.getBottomLeft().getX()>=0 && x.getBottomLeft().getY()>=0)
                .map(rec -> {
                    Rectangle recMirror = rec.mirror(rec);
                    rec = recMirror.scale(50D);
                    return rec;
                })
