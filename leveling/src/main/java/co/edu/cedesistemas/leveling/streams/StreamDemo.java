...
public class StreamDemo {
    ...
    {
        List<Shape> resultList = shapes.stream().filter(i -> areaLimit >= i.area()).collect(Collectors.toList());
        return resultList;
    }
    ...
    {
        ShapeMultiplier<T, U> multiplier = (s,u) -> s.scale(u);

        List<T> list = scalables.stream().map(escala -> multiplier.multiply(escala, value)).collect(Collectors.toList());
        return list;
