...
public class OrderController {
    ...
    public ResponseEntity<Status<?>> findOrderById(@PathVariable String id){
        try {
    ...
    public ResponseEntity<Status<?>> findOrderItems(@PathVariable String id){
        try {
            List<OrderItem> found = service.getItemsByOrderId(id);
            if (found != null) return DefaultResponseBuilder.defaultResponse(found, HttpStatus.OK);
            else return DefaultResponseBuilder.errorResponse("OrderItems not found", null, HttpStatus.NOT_FOUND);
        } catch (Exception ex) {
            return DefaultResponseBuilder.errorResponse(ex.getMessage(), ex, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
