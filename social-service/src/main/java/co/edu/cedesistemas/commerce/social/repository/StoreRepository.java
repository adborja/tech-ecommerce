...
public interface StoreRepository extends Neo4jRepository<Store, String> {
    ...
    @Query("MATCH (user:User {id: $userId})-[:IS_FRIEND_OF]-(friend)-[:LIKES]->(store:Store)-[:LOCATED_IN]->(loc:Location {zone: $zone}), (store)-[:SERVES]->(type:ProductType {name: $productType}) RETURN store.id as storeId, count(*) AS occurrence, loc.zone as zone ORDER BY occurrence DESC LIMIT $limit")
    List<StoreOccurrence> findRecommendationByStores(@Param("userId") String userId, @Param("zone") String zone,
                                                     @Param("productType") String productType,
                                                     @Param("limit") Integer limit);

    //stores that user's friends likes
